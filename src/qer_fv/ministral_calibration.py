"""Shared, auditable calibration rendering for both Ministral INT4 routes."""
...
def ministral_tokenizer_load_kwargs() -> dict[str, bool]:
    """Return the frozen local tokenizer policy for Ministral 3 checkpoints."""
    return {
...
def render_ministral_calibration(
    ...
    for row in rows:
        ...
        text = tokenizer.apply_chat_template(rendered.messages, tokenize=False, **kwargs)
        input_ids = normalize_chat_template_token_ids(
            tokenizer.apply_chat_template(rendered.messages, tokenize=True, **kwargs)
        )
        if not isinstance(text, str) or not text or not input_ids:
            raise ValueError("calibration chat-template rendering is invalid")
